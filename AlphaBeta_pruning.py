...
def alpha_beta_pruning(board, depth, alpha, beta, maximizing_player):
    global search_nodes, pruned_nodes
    search_nodes += 1  
...
def play_game():
    ...
    while not is_terminal_state(board):
        ...
        if turn == "X": 
            ...
            print(f"Player move time: {elapsed_time:.4f} seconds")
        else:  
            print("AI is thinking...")
            start_time = time.time()
            search_nodes = pruned_nodes = 0  
            _, ai_move = alpha_beta_pruning(board, depth=3, alpha=float('-inf'), beta=float('inf'), maximizing_player=True)
            elapsed_time = time.time() - start_time
            print(f"AI move time: {elapsed_time:.4f} seconds")
            print(f"Nodes searched: {search_nodes}, Nodes pruned: {pruned_nodes}")
            if ai_move:
    ...
    if is_terminal_state(board):
        print("Congratulations! You win!！" if turn == "O" else "Sadly, AI wins!")
    else:
        print("draw！")
